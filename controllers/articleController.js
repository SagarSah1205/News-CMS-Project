...
const addArticle = async (req, res, next) => {
  ...
  try {
    ...
    const article = new newsModel({
      ...
      content,
      category,
      author: req.id, // Assuming you have the user ID available in the request
    ...
    // console.log(error);
    // res.status(500).send('Article not saved. Please try again.');
    next(error);
  }
}
...
const updateArticle = async (req, res, next) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      article.image = req.file.filename; // Update the image if a new one is uploaded
    }
